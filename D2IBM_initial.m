function [IB_Coord,Euler_Index,Euler_Coord, Euler_Dirac, EuNeighbor_Index, Num_Lagrangian] = D2IBM_initial(Eu_radius, Arc_Length)
...
for i = 1:Num_Lagrangian
    ...
    LocLag(1,1) = xo + r_solid*cos(theta_seperated*(i-1)) ;
    LocLag(1,2) = yo + r_solid*sin(theta_seperated*(i-1)) ;

    IB_Coord{i} = LocLag;

    Vertor_IB(i,1) = xo + r_solid*cos(theta_seperated*(i-1)) ;
    Vertor_IB(i,2) = yo + r_solid*sin(theta_seperated*(i-1)) ;
...
for j=jmin: jmax
    for i=imin: imax
        if sqrt((Coord_x(i)- xo)^2 + (Coord_y(j)-yo)^2) < (r_solid + 5*dx) && ...
           sqrt((Coord_x(i)- xo)^2 + (Coord_y(j)-yo)^2) > (r_solid - 5*dx) 
