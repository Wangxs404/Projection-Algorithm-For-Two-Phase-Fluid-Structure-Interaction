...
dt=1e-4     ;    istep_max=ceil(20/dt);  theta=90;  % 默认润湿角
FsTension="Off";    %控制开关Fs
Wetting="Off"; Thermal="Off";  Surfactant="Off";      % 选择多物理场（"ON"）
Electronic="Off"; Magnetic="Off"; ImmerseBoundaryCondition="ON";
Res_Freq=50  ;  dat_Freq=1000;  mat_Freq=2000;  Dynamic_Draw='ON'; % Debug设置
...
%==================================将数据存储至上级目录=============
name="RisingBubble_IBM_10_DensityDirac-Y1.5";        % 在上级目录创建文件夹存储数据
...
% === IBM 拉格朗日点 初始化===
Arc_Length = 1.2*dx;         % 参考Zhu Yang的博士论文
Eu_radius = 1.8*sqrt(2)*dx;  % 邻域Euler半径,每个La点约19~21个Eu点
...
A_IB = (rho_Light+rho_Heavy)/2 * Arc_Length * dt * dx^2 * (DiracMatrix * DiracMatrix');
...
istep=0;                                 %===为便于续算，需将初始istep置于 SpecialSet”前
 ...
 %====续算/Cases个性化=====绘图、输出/保存初始数据%===指定无量纲量并输出
        ...
        run D2SpecialSet;    run D2DimLess ;  
%         run D2PostIni
...
switch Dynamic_Draw
    case 'ON'
        if     mod(istep,20) ==  0
            ...
            % contourf(Coord_x(4:1+nx),Coord_y(4:1+ny),u(4:1+nx,4:1+ny)') %等高线图
