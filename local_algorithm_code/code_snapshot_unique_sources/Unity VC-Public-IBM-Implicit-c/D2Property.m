...
switch property  %常规
    case 1
        ...
        gx=0   ;     gy=-0.98*10;                 % g向上为正
        M=1e-2 ;     epsilon=0.02;      sigma0=100;        %迁移率 界面厚度 表面张力系数
    case 2
        mu_Light=0.1  ;  mu_Heavy=10  ;  rho_Light =1  ;  rho_Heavy=1000; % “-1”represent “the Heavy one”
        gx=0   ;     gy=-0.98;                 % g向上为正
        M=1e-2 ;     epsilon=1.2e-2;      sigma0=1.96;        %迁移率 界面厚度 表面张力系数
    case 3    %用于质量泄漏测试
        mu_Light=0.01  ;  mu_Heavy=0.01  ;  rho_Light =1 ;  rho_Heavy=1; % "-1"represent "the Heavy one"
        gx=0   ;     gy=0;                 % g向上为正
        M=1e-2 ;     epsilon=0.02;      sigma0=0.1;          %迁移率 界面厚度 表面张力系数
    case 31    %用于磁场测试
    ...
    case 4   %用于质量泄漏测试
        mu_Light=0.001  ;  mu_Heavy=0.01  ;  rho_Light =1  ;  rho_Heavy=1000; % “-1”represent “the Heavy one”
        gx=0   ;     gy=-0.98;                 % g向上为正
        M=1e-5 ;     epsilon=1e-2;      sigma0=1.96;        %迁移率 界面厚度 表面张力系数
    case 5 %real
        mu_Light = 1.78e-5 ;  mu_Heavy = 1.002e-3 ;  rho_Light = 1.204 ;  rho_Heavy=998.207; % “-1”represent “the Heavy one”
        gx=0   ;     gy=-9.8;                 % g向上为正
        M=1e-8;     epsilon=0.02*Dim;      sigma0=7.28e-2;        %迁移率 界面厚度 表面张力系数
    case 6 %剪切流液滴变形
        ...
        M=0.01;     epsilon=2e-2;       %迁移率 界面厚度 表面张力系数
   case 7 %双液滴融合
        mu_Light=1  ;  mu_Heavy=10  ;  rho_Light =100  ;  rho_Heavy=1000; % “-1”represent “the Heavy one”
        gx=0   ;     gy=0;                 % g向上为正
...
case 10 %双液滴融合
        mu_Light=0.00195  ;  mu_Heavy=0.0013  ;  rho_Light =1  ;  rho_Heavy=1.5; % “-1”represent “the Heavy one”
        gx=0   ;     gy=-9.8;                 % g向上为正
        M=1e-2 ;     epsilon=1e-2;      sigma0=0;  
end
